...
class Statistics:
    ...
    game_entry_percentage: List[PercentageItem]
    ...
    async def hourly_battery_usage_statistics(self):
        ...
        # Step 2: Count the total number of entries
        ...
        if total_entries > 0:
            ...
            # Sort games by percentage in descending order to distribute the rounding error
            sorted_games = sorted(
                game_entry_percentage.items(), key=lambda x: x[1], reverse=True
            )

            # Distribute the rounding error proportionally
            for i in range(int(rounding_error)):
                game, percentage = sorted_games[i % len(sorted_games)]
            ...
            self.game_entry_percentage = game_entry_percentage_list
